...
#	if (Input.is_action_just_pressed("key_left") or 
#	Input.is_action_just_pressed("key_right") or 
#	Input.is_action_just_pressed("key_up") or 
#	Input.is_action_just_pressed("key_down")):
#		move_direction = Input.get_vector("key_left", "key_right", "key_up", "key_down")
#	player.direction = run_direction
...
#	Global.print_timed(friction_force, 0.1)	
...
#	Global.print_timed(str(current_speed) + "		" + str(current_direction), 0.1)
...
func _process(delta):
#	Global.print_timed(get_node("..").velocity.length(), 0.1)
...
#	Global.print_timed(snapped(av_spd, 0.01), 0.1)
